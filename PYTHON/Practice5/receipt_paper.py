...
product_names = re.findall(product_pattern, text, re.MULTILINE)

# 3. Calculate total amount from item totals
item_total_pattern = r"x\s+[\d ]+,\d{2}\n([\d ]+,\d{2})"
...
receipt_total = normalize_price(total_match.group(1)) if total_match else None
...
result = {
    ...
    "total_is_correct": calculated_total == receipt_total,
    "date": date,
    "time": time,
